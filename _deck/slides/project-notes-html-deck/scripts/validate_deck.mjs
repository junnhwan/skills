...
const fontSizeProps = [...html.matchAll(/font-size\s*:\s*([^;]+);/gi)].map((match) => match[1]);
...
for (const prop of fontSizeProps) {
  const pixelValues = [...prop.matchAll(/(\d+(?:\.\d+)?)px/gi)].map((match) => Number(match[1]));
  const maxPx = Math.max(0, ...pixelValues);
  if (maxPx > 72) {
    failures.push(`oversized font-size ${maxPx}px found; source/interview notes should stay <= 72px`);
    break;
  }
}
